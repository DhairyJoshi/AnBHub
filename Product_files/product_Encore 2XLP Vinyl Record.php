<?php
    ...
    if ($eDate < $time)  
    {
        if ($highestBidder == $_SESSION['Username'])   
        {
            $sql = "UPDATE products SET status='sold' WHERE Name='$fileName'";  
            $query = mysqli_query($conn, $sql);
            // header ("location:https://rzp.io/l/jIYPudoot");
            $showAlert = "Make Payment";
            $showPayment = $highestBidder . " make payment of $" . $currentBid;
        }
        else
        {
            $showAlert = "Product sold";
            $showError = "Product was sold to someone else";
        }
    }
?>
                    ...
                    <div class='notification'>
                        <div class='content flex justify-between bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3'>
                            <div class='flex'>
                                ...
                                <div>
                                    <p class='font-bold'>" . $showAlert . "</p>
                                    <p class='text-sm'>" . $showPayment . "</p>
                                </div>
                            </div>
                            <div class='button'>
                                <button class='button-green' role='button'><a href='https://rzp.io/l/jIYPudoot'>Next</a></button>
                            </div>
                    </div>
                </div>";
            }
